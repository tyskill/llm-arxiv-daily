...
def load_config(config_file: str) -> dict:
    ...
    def pretty_filters(**config) -> Dict:
        ...
        OR_LITERAL = ' OR '
        AND_LITERAL = ' AND '
        def parse_filters(filters: List[str]) -> str:
            ret = ''
            for idx in range(0, len(filters)):
                use_and = False
                filter = filters[idx]
                if filter.startswith('\"') and filter.endswith('\"'):
                    # 如果以引号开头和结尾，则表示作为 AND 条件使用
                    use_and = True
                elif len(filter.split()) > 1:
                    # if filter contains whitespace characters, it needs to be escaped.
                    ret += (EXCAPE + filter + EXCAPE)
                else:
                    ret += (QUOTA + filter + QUOTA)
                if (idx != len(filters) - 1) and not use_and:
                    ret += OR_LITERAL
                elif use_and:
                    # filter 已经包含引号，因此不需要再套引号
                    ret = ret.removesuffix(OR_LITERAL)
                    if ret != '':
                        ret += (AND_LITERAL + filter)
                    else:
                        ret += (filter)
...
def get_daily_papers(topic: str, query="Prompt Injection", max_results=2):
    """
    @param topic: str
    @param query: str
